...
const requiredFiles = [
  ...
  'components/LocalSecurityHubCurrent.tsx',
  'components/LocalSecurityPortal.tsx',
  'lib/local-security-current.ts',
  'lib/local-east-news-current.ts',
...
for (const check of checks) {
  const content = text(check.file);
  for (const token of check.forbidden) if (content.includes(token)) failures.push(`${check.file}: rótulo/estado proibido “${token}”. ${check.reason}`);
}
...
for (const token of ['21 AGO 2026','aricanduva-homicidio-20ago','ponte-rasa-oficina-20ago','sao-mateus-motorista-refem-19ago']) {
  if (!localCurrent.includes(token)) failures.push(`lib/local-security-current.ts: feed recente incompleto; ausente “${token}”.`);
}
const localEast = text('lib/local-east-news-current.ts');
for (const token of ['21 AGO 2026','Corinthians vence Rosario Central','Córrego Rio Verde','vacinação contra sarampo']) {
  if (!localEast.includes(token)) failures.push(`lib/local-east-news-current.ts: pauta regional atual incompleta; ausente “${token}”.`);
}
...
for (const token of ['grid-template-columns:minmax(0,1fr)!important','flex-direction:column!important','.zlCurrentNewsGrid']) {
  if (!mobile.includes(token)) failures.push(`app/mobile-v10.css: hardening mobile incompleto; ausente “${token}”.`);
}
...
if (cssImports.at(-1) !== 'mobile-v10.css') failures.push(`app/layout.tsx: mobile-v10.css deve ser o último CSS para vencer regras legadas; último atual: ${cssImports.at(-1) ?? 'nenhum'}.`);

// HARD GATE diário: 22 áreas precisam ter sido revistas no dia corrente de São Paulo.
