...
type Theme = 'conversation'|'challenge'|'science'|'psychology'|'technology'|'pregnancy'|'parenthood'|'travel'|'music'|'games'|'finance'|'security'|'cars'|'motos'|'mechanic'|'world'|'weather'|'football';
...
security:'https://upload.wikimedia.org/wikipedia/commons/thumb/c/c4/Gitea_Logo_with_Wordmark.svg/1920px-Gitea_Logo_with_Wordmark.svg.png',
cars:'https://commons.wikimedia.org/wiki/Special:Redirect/file/Chevrolet%20Cruze%202014%20facelift%20IMG002.jpg',
motos:'https://commons.wikimedia.org/wiki/Special:Redirect/file/HONDA%20CBX%20250%20TWISTER.jpg',
...
football:'https://s2-cbn.glbimg.com/P_wXvQ-Ks8UHdDwWGCCglRTv52A=/1200x/smart/filters:cover()/filters:strip_icc()/i.s3.glbimg.com/v1/AUTH_d975fad146a14bbfad9e763717b09688/internal_photos/bs/2024/z/7/dl7DuXRIuwuFzGi0fAaQ/corinthian.png'
};
...
function themeFor(reel:Reel):Theme{if(reel.theme)return reel.theme;if(reel.slug==='gravidez')return'pregnancy';if(reel.slug==='pai')return'parenthood';if(reel.slug==='viagens')return'travel';if(reel.slug==='musica')return'music';if(reel.slug==='games')return'games';if(reel.slug==='financas')return'finance';if(reel.slug==='security-briefing'||reel.slug==='seguranca'||reel.slug==='appsec-ssdlc')return'security';if(reel.slug==='carros')return'cars';if(reel.slug==='motos')return'motos';if(reel.slug==='mecanica')return'mechanic';if(reel.slug==='mundo')return'world';if(reel.slug==='tempo')return'weather';if(reel.slug==='corinthians')return'football';return'science';}
...
function journalisticReel(slug:string){if(freshnessForSlug(slug)?.state!=='ATUALIZADO')return undefined;const reel=categoryReel(slug);if(!reel?.image)return undefined;return reel;}
...
const SERTANEJO:Reel={slug:'sertanejo-da-roca',label:'Sertanejo da Roça',emoji:'🪕',kind:'special-media',title:'Clássico da roça do dia: Tristeza do Jeca',detail:'Tonico e Tinoco interpretam o clássico de Angelino de Oliveira em registro do acervo da TV Cultura. Hoje o foco é a tradição da música caipira e a história dessa interpretação — sem reproduzir a letra.',conversation:'Qual clássico de viola ou música caipira você lembra de ouvir na família?',videoEmbed:'https://www.youtube.com/embed/Fxac2CJDo8A?playsinline=1&rel=0',content:{badge:'SERTANEJO DA ROÇA · 26/08',title:'Tristeza do Jeca',summary:'Clássico caipira em rotação diária, com vídeo legítimo e contexto histórico.',sections:[],sources:[{label:'TV Cultura · Tristeza do Jeca, por Tonico e Tinoco',url:'https://www.youtube.com/watch?v=Fxac2CJDo8A'}]}};
function buildCorinthians():Reel|undefined{const title='Corinthians monitora joelho de Hugo Souza antes do clássico contra o Santos';const image=getImage('Corinthians',title);if(!image)return undefined;return{slug:'corinthians',label:'Corinthians',emoji:'⚽',kind:'editorial',theme:'football',title,detail:'Hugo Souza será reavaliado nesta quarta após dores no ligamento colateral medial. A tendência é de retorno contra o Santos no domingo, às 16h, na Neo Química Arena, mas o clube pode preservar o goleiro se o incômodo persistir.',image,content:{badge:'CORINTHIANS · 26/08',title,summary:'Atualização médica e de escalação do dia.',sections:[{title:'Situação de Hugo Souza',bullets:['O goleiro será reavaliado no CT Joaquim Grava nesta quarta-feira.','O Corinthians não trata o quadro como grave, mas quer evitar risco de agravamento.','Se as dores persistirem, ele pode ser novamente preservado.']},{title:'Próximo jogo oficial',bullets:['Corinthians x Santos · domingo, 30/08 · 16h · Neo Química Arena · 25ª rodada do Brasileirão.','Breno Bidon está suspenso pelo terceiro cartão amarelo.','Rodrigo Garro volta após cumprir suspensão na rodada anterior.']},{title:'Último resultado relevante',paragraphs:['O Corinthians perdeu para o Coritiba por 2 a 1 na rodada anterior do Brasileirão e terminou o jogo com 32 pontos.']},{title:'O que acompanhar',bullets:['Resultado da reavaliação de Hugo Souza.','Lista final de relacionados e evolução dos demais lesionados.','Definição da escalação para o clássico.']}],sources:[{label:'ge · Hugo Souza é reavaliado antes de Corinthians x Santos · 26/08/2026',url:'https://ge.globo.com/futebol/times/corinthians/noticia/2026/08/26/corinthians-evita-risco-e-monitora-dores-no-joelho-de-hugo-souza-ate-jogo-contra-o-santos.ghtml'},{label:'ge · Breno Bidon suspenso contra o Santos',url:'https://ge.globo.com/futebol/times/corinthians/noticia/2026/08/23/breno-bidon-recebe-o-terceiro-amarelo-e-desfalca-o-corinthians-no-classico-contra-o-santos.ghtml'}]}};}
...
export default function ReelsExperienceV26(){
  ...
  const reels=useMemo<Reel[]>(()=>{
    ...
    return[buildTalk(),buildChallenge(),...curiosityReels(),...family,...news,...(corinthians?[corinthians]:[]),...lifestyle,SERTANEJO,...tech,...auto].filter(mediaReady);
  },[]);
