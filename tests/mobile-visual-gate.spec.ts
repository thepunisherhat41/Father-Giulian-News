...
test('current edition is healthy on mobile card by card', async ({ page }) => {
  ...
  for(const required of ['31 ago 2026','lua se afasta da terra','córnea é transparente','ssds usam wear leveling','8 semanas + 1 dia','caixa conclui hoje o bolsa família','nis final 0','tse abre compilação e lacração','assinatura digital','são paulo chega ao último dia de agosto','32°c','baixa umidade','irã rejeita alegação de ataque a kharg','terminal segue operacional','cate móvel atende hoje','casa da mulher em itaquera','cruzeiro x corinthians abre hoje','19h15','europa amplia rede de ia','lumi-ai','fsb coloca risco cibernético','último dia do novo desenrola','carro usado até r$70 mil','bateria fraca pode parecer']) expect(pageText).toContain(required.toLowerCase());
