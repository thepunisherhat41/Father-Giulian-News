...
test('current edition is healthy on mobile card by card', async ({ page }) => {
  ...
  for(const required of ['31 ago 2026','lua se afasta da terra','córnea é transparente','ssds usam wear leveling','8 semanas + 1 dia','justiça do rio decreta falência do grupo refit','bloqueio das contas bancárias','votação da mp da “taxa das blusinhas” fica para terça','tse abre escuta sobre experiência do eleitor','btg/nexus','flávio bolsonaro 33%','augusto cury 11%','br-08900/2026','são paulo chega ao meio-dia','22,2°c','umidade alta','chance de chuva','irã rejeita alegação de ataque a kharg','terminal segue operacional','cate móvel atende hoje','casa da mulher em itaquera','vic decide no fim','cruzeiro 0 x 1 corinthians','50 minutos do segundo tempo','jhonson','europa amplia rede de ia','lumi-ai','fsb coloca risco cibernético','último dia do novo desenrola','carro usado até r$70 mil','bateria fraca pode parecer']) expect(pageText).toContain(required.toLowerCase());
  for(const omitted of ['30 ago 2026','kharg segue sem confirmação','eua e irã retomam ataques; alegação','céu fica azul','ponto cego','qr code','caixa conclui hoje o bolsa família','nis final 0','corinthians repudia agressões contra criança','brabas jogam às 19h15','cruzeiro x corinthians abre hoje as quartas','náutica']) expect(pageText).not.toContain(omitted.toLowerCase());
