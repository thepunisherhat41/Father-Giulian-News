...
test('current edition is healthy on mobile card by card', async ({ page }) => {
  ...
  for(const required of ['31 ago 2026','lua se afasta da terra','córnea é transparente','ssds usam wear leveling','8 semanas + 1 dia','caixa conclui hoje o bolsa família','nis final 0','tse abre compilação e lacração','assinatura digital','são paulo chega ao meio-dia','22,2°c','umidade alta','chance de chuva','irã rejeita alegação de ataque a kharg','terminal segue operacional','cate móvel atende hoje','casa da mulher em itaquera','corinthians repudia agressões contra criança','brabas jogam às 19h15','europa amplia rede de ia','lumi-ai','fsb coloca risco cibernético','último dia do novo desenrola','carro usado até r$70 mil','bateria fraca pode parecer']) expect(pageText).toContain(required.toLowerCase());
  for(const omitted of ['30 ago 2026','kharg segue sem confirmação','eua e irã retomam ataques; alegação','céu fica azul','ponto cego','qr code','bombeiros resgatam 20 pessoas','presidenciáveis cumprem agendas de domingo','santos vence o corinthians por 1 a 0','gamescom 2026 chega ao último dia','são paulo chega ao último dia de agosto com calor de até 32°c','cruzeiro x corinthians abre hoje as quartas','náutica']) expect(pageText).not.toContain(omitted.toLowerCase());
